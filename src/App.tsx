...
import { BotaoIcone } from './componentes/ui/BotaoIcone'
  ...
  return (
    ...
    >
      <div className="flex h-full flex-col">
        ...
        */}
        <header className="flex flex-wrap items-center justify-between gap-x-2 gap-y-1.5 border-b bg-background px-3 py-2 sm:flex-nowrap sm:gap-6 sm:px-4">
          <div className="flex min-w-0 items-center gap-2">
            <Logo size={22} className="shrink-0" />
            <span className="truncate text-[15px] font-semibold tracking-tight">Phrame</span>
            <span className="shrink-0 rounded-md border px-1.5 py-px font-mono text-[10px] text-muted-foreground tabular-nums">
              v{__VERSAO__}
            </span>
          ...
          <div className="flex w-full items-center justify-end gap-0.5 sm:w-auto sm:gap-1">
            <BotaoIcone dica="Desfazer (Ctrl+Z)" onClick={desfazer} disabled={!podeDesfazer}>
              <Undo2 />
            </BotaoIcone>
            <BotaoIcone dica="Refazer (Ctrl+Shift+Z)" onClick={refazer} disabled={!podeRefazer}>
              <Redo2 />
            </BotaoIcone>
            <Separator orientation="vertical" className="mx-1.5 hidden h-5! sm:block" />
            <BotaoIcone
              dica={tema === 'claro' ? 'Mudar para o tema escuro' : 'Mudar para o tema claro'}
              onClick={alternarTema}
            >
              {tema === 'claro' ? <Moon /> : <Sun />}
            </BotaoIcone>
            <BotaoIcone dica="Recomeçar do zero" onClick={() => setConfirmarRecomeco(true)}>
              <RotateCcw />
            </BotaoIcone>
            <Separator orientation="vertical" className="mx-1.5 hidden h-5! sm:block" />
            <BotaoExportar />
      ...
      <DragOverlay dropAnimation={null}>
        {imagemArrastada && (
          <img
            ...
            alt=""
            className="h-24 w-24 rounded-lg border-2 border-primary object-cover shadow-2xl"
          />
        )}
      </DragOverlay>

      <AlertDialog open={confirmarRecomeco} onOpenChange={setConfirmarRecomeco}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Recomeçar do zero?</AlertDialogTitle>
            <AlertDialogDescription>
              A colagem, as lâminas e as fotos carregadas serão descartadas. Não dá para desfazer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction variant="destructive" onClick={limparTudo}>
              Descartar tudo
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
